...
export class VacancyList implements OnInit {
  ...
  readonly statusOptions=[{label:'All statuses',value:null as PositionStatus|null},{label:'Open',value:'OPEN' as PositionStatus},{label:'Filled',value:'FILLED' as PositionStatus},{label:'Closed',value:'CLOSED' as PositionStatus}];
  ...
  openDetails(v:Position):void{this.selectedDetails.set(null);this.detailsLoading.set(true);this.detailsVisible.set(true);this.api.get(v.id).pipe(finalize(()=>this.detailsLoading.set(false))).subscribe({next:(data)=>this.selectedDetails.set(data),error:()=>this.messages.add({severity:'error',summary:'Unable to load vacancy details',detail:'Please try again.'})})}viewPosition(v:Position):void{this.detailsVisible.set(false);this.router.navigate(['/positions'],{queryParams:{q:v.title}})}invalid(name:string):boolean{const c=this.form.get(name);return!!c&&c.invalid&&(c.touched||c.dirty)}statusLabel(s:PositionStatus):string{return s==='OPEN'?'Open':s==='FILLED'?'Filled':'Closed'}statusSeverity(s:PositionStatus):'warn'|'success'|'secondary'{return s==='OPEN'?'warn':s==='FILLED'?'success':'secondary'}formatDate(v?:string):string{return v?new Date(v).toLocaleDateString():'—'}
